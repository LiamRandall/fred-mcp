  ...
  server.tool(
    ...
    async (args: ParamsType): Promise<CallToolResult> => {
      try {
        ...
        console.error(`Error executing get_fred_series_vintagedates:`, error);
        return {
          content: [
            {
              type: "text",
              text: `Error executing get_fred_series_vintagedates: ${error instanceof Error ? error.message : String(error)}`,
            },
          ],
        };
      }
