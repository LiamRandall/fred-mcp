...
export function setupTool<S extends UpstreamMCPServer>(server: S) {
  const params = {
    "Description:": z.string().optional().describe("Get an economic data series."),
  ...
  server.tool(
    ...
    async (args: ParamsType): Promise<CallToolResult> => {
      try {
        ...
        console.error(`Error executing get_fred_series_categories:`, error);
        return {
          content: [
            {
              type: "text",
              text: `Error executing get_fred_series_categories: ${error instanceof Error ? error.message : String(error)}`,
            },
          ],
        };
      }
