  ...
  server.tool(
    ...
    async (args: ParamsType): Promise<CallToolResult> => {
      try {
        ...
        console.error(`Error executing get_fred_category_children:`, error);
        return {
          content: [
            {
              type: "text",
              text: `Error executing get_fred_category_children: ${error instanceof Error ? error.message : String(error)}`,
            },
          ],
        };
      }
