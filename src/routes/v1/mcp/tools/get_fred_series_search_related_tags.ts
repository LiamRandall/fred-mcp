...
export function setupTool<S extends UpstreamMCPServer>(server: S) {
  ...
  server.tool(
    "get_fred_series_search_related_tags",
    ...
    async (args: ParamsType): Promise<CallToolResult> => {
      try {
        ...
        console.error(`Error executing get_fred_series_search_related_tags:`, error);
        return {
          content: [
            {
              type: "text",
              text: `Error executing get_fred_series_search_related_tags: ${error instanceof Error ? error.message : String(error)}`,
            },
          ],
        };
      }
